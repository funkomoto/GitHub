...
def pietwo():
  ...
  while check != s:
    ...
    p = Decimal(2)*p
    n += 1
    print('{0:.100f}'.format((a + b)**Decimal(2) / (Decimal(4) * t)))
...
def piethreex():
  print('{0:.100f}'.format((Decimal(16)*arctan(Decimal(1)/Decimal(5))) - (Decimal(4)*arctan(Decimal(1)/Decimal(239)))))
...
## Tan calculator (BROKEN FOR SOME NUMBERS)
...
mode = input ("\nRadians or degrees: ")
if mode in ["calc pi long", "calc long pi", "calc pi", "calculate pi"]:
